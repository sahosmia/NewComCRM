...
trait ValidatesRequirementAttributes
{
    ...
    protected function requirementAttributeRules(): array
    {
        return [
            ...
            'accessories.*.title' => 'required|string|max:255',
            'accessories.*.quantity' => 'required|integer|min:1',
            'accessories.*.unit_id' => 'required|exists:units,id',
            ...
            'installations.*.title' => 'required|string|max:255',
            'installations.*.quantity' => 'required|integer|min:1',
            'installations.*.unit_id' => 'required|exists:units,id',
            'installations.*.price' => 'required|numeric|min:0',
