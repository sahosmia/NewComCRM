...
export default function Edit({ requirement, customers, products, units, users, companies }: Props) {
    return (
        <AppLayout breadcrumbs={[{ title: "Requirements", href: route('requirements.index') }, { title: "Edit", href: route('requirements.edit', requirement.id) }]}>
            ...
            <div className="p-6">
                ...
                <div className="bg-card p-6 border rounded-lg shadow-sm">
                    <RequirementForm requirement={requirement} customers={customers} products={products} units={units} users={users} companies={companies} />
                </div>
            </div>
        </AppLayout>
    );
